...
if (!response.ok) throw new Error(`Erro ao carregar: ${response.statusText}`);
            ...
            produtosContainer.innerHTML = `<p>Falha ao carregar produtos.
...
if (itemExistente) {
            if (itemExistente.quantidade < LIMITE_POR_ITEM) {
                ...
                mostrarFeedback(`Limitado a ${LIMITE_POR_ITEM} itens por cliente`); // <-- ALTERAÇÃO AQUI
return;
            }
        } else {
...
return;
        ...
        carrinho.forEach(item => {
            [cite_start]const precoUnitario = item.PRECO.toFixed(2).replace('.', ','); [cite: 40, 41]
            [cite_start]const subtotal = (item.PRECO * item.quantidade).toFixed(2).replace('.', ','); [cite: 40, 41]
            // Formato: [Nome do Produto] - R$ [valor unitário] x [quantidade] x R$ [subtotal do produto]
            [cite_start]mensagem += `${item.DESCRICAO} - R$ ${precoUnitario} x ${item.quantidade} x R$ ${subtotal}\n`; [cite: 40, 41]
        });

        [cite_start]const total = carrinho.reduce((sum, item) => sum + (item.PRECO * item.quantidade), 0); [cite: 42]
        [cite_start]mensagem += `\nValor Total: R$ ${total.toFixed(2).replace('.', ',')}`; [cite: 43]
