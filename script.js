document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderizarProdutos() {
        ...
        if (termoPesquisa) {
            ...
            produtosFiltrados = produtosFiltrados.filter(p =>
                // Procura em todos os valores do objeto produto
        ...
        produtosFiltrados.forEach(produto => {
            ...
            card.innerHTML = `
                <div class="produto-imagem-container">
                    ...
                    <img class="produto-imagem" src="${produto.IMAGES[0]}" alt="${produto.DESCRICAO}" loading="lazy" onerror="this.style.display='none'; this.previousElementSibling.textContent='Imagem indisponível';">
                </div>
    ...
    function atualizarCarrinho() {
        ...
        if (carrinho.length === 0) {
            ...
            finalizarPedidoBtn.disabled = true; // Desabilita o botão se o carrinho estiver vazio
            return;
        }
        
        finalizarPedidoBtn.disabled = false; // Habilita o botão se houver itens
        let totalGeral = 0;
        let totalItens = 0;
        ...
        carrinho.forEach(item => {
            ...
            // CORREÇÃO: trocado o <span> de remover por um <button> para semântica e acessibilidade.
            itemDiv.innerHTML = `
                ...
                <div class="carrinho-item-detalhes">
                    ...
                    <div class="carrinho-item-controles">
                        ...
                        <input type="number" class="quantidade-input" value="${item.quantidade}" min="1" data-sku="${item.SKU}" aria-label="Quantidade">
                        <button class="quantidade-btn" data-sku="${item.SKU}" data-action="incrementar" aria-label="Aumentar quantidade">+</button>
                        <button class="remover-item" data-sku="${item.SKU}">Remover</button>
                    </div>
                </div>
            `;
    ...
    function toggleCarrinho() {
        const isOpen = carrinhoSidebar.classList.toggle('open');
    ...
    function handlePesquisa() {
        termoPesquisa = pesquisaInput.value.trim(); // Mantém o case para filtros futuros se necessário, mas a busca é case-insensitive
        limparPesquisaBtn.style.display = termoPesquisa ? 'grid' : 'none'; // 'grid' para usar 'place-content'
        renderizarProdutos();
    }
    ...
    function handleFormSubmit(e) {
        ...
        // Fecha o carrinho apenas se ele estiver aberto
    ...
    function adicionarEventListeners() {
        ...
        produtosContainer.addEventListener('click', e => {
            if (e.target.closest('.add-carrinho')) {
                adicionarAoCarrinho(e.target.closest('.add-carrinho').dataset.sku);
            }
        });
        ...
        carrinhoContent.addEventListener('click', e => {
            ...
            if (!sku) return; // Sai da função se não houver SKU
            ...
            if (target.classList.contains('quantidade-btn')) {
                ...
                if (target.dataset.action === 'incrementar') {
                    qtde++;
                } else if (target.dataset.action === 'decrementar') {
                    qtde--;
                }
        ...
        carrinhoContent.addEventListener('change', e => {
            if (e.target.classList.contains('quantidade-input')) {
                // Converte para número e garante que não seja inválido (NaN)
                const novaQuantidade = parseInt(e.target.value, 10) || 1;
                atualizarQuantidade(e.target.dataset.sku, novaQuantidade);
            }
        });
