...
// CORREÇÃO NO BOTÃO VOLTAR (LIFO - Last In First Out)
window.addEventListener('popstate', (event) => {
    ...
    // 2. Prioridade: Fechar Modal de Pedido
    ...
    // 3. Prioridade: Fechar Carrinho
    if (carrinhoAberto) {
        toggleCarrinho(false); 
        return; // PARA A EXECUÇÃO AQUI
    }
    
    // 4. Se não tinha nada aberto, Reseta Categoria/Pesquisa e volta para Home
...
function gerarBotoesCategoria() {
    ...
    todosBtn.addEventListener('click', () => {
        // Se estiver em sub-estado, volta para home
...
function toggleCarrinho(fromUserAction = true) {
    ...
    if(carrinhoAberto) {
        ...
        // Se fechou pelo X ou clique fora, volta o histórico
...
formDadosCliente.addEventListener('submit', function(e) {
    ...
    // 2. Limpa o Carrinho e Fecha Modais ANTES de abrir o WhatsApp
    carrinho = [];
    atualizarCarrinho(); // Salva vazio no localStorage
    ...
    // Volta o histórico se necessário (para remover o estado 'inner' do modal)
    if (history.state && history.state.page === 'inner') {
        history.back();
    }
