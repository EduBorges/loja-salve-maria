...
window.addEventListener('popstate', (event) => {
    ...
    // 4. Se não tinha nada aberto, Reseta Categoria/Pesquisa
    if (categoriaAtiva !== 'todos' || termoPesquisa !== '') {
        document.querySelectorAll('.categoria-btn').forEach(b => b.classList.remove('active'));
        // Busca o botão Todos recém criado ou existente
...
function finalizarPedidoWhatsApp(e) {
    // Stop propagation e prevent default para não fechar modal instantaneamente
    if(e) {
        e.preventDefault();
        e.stopPropagation();
    }
...
document.addEventListener('click', (e) => {
    // Se o modal estiver aberto, ignoramos cliques para fechar o sidebar
